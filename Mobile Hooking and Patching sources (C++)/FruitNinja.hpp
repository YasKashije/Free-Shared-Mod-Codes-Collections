...
//SVInventory - GetCount(string itemName)
int (*orig_GetCount)(void* _this, monoString *item); 
int GetCount(void* _this, monoString *item) {
	const char *getItemChar = monoStringConvert(item);

	//if (strcmp(getItemChar, "coin") == 0)
	//{
	//	return 99999999;
	//}		
	if (strcmp(getItemChar, O("key")) == 0)
	{
		LOGD(OBF("return"));
		return 2000;
	}
	return orig_GetCount(_this, item); 
}
